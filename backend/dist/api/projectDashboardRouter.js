...
exports.createProjectDashboardRouter = createProjectDashboardRouter;
exports.createProjectErrorUpsertRouter = createProjectErrorUpsertRouter;
exports.createProjectLiveRouter = createProjectLiveRouter;
 ...
 */
function buildTotalUnion(tables) {
    const parts = tables.map((t) => `SELECT REPLACE(project_name, '_', ' ') AS project_name, COUNT(*) AS cnt FROM "${t}" GROUP BY project_name`);
    return parts.join('\n UNION ALL\n');
}
// ─── Router factory ───────────────────────────────────────────────────────────
function createProjectDashboardRouter(pool) {
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    const express = require('express');
    const router = express.Router();
    // GET /top-projects — top 10 projects by total row count
    router.get('/top-projects', async (_req, res) => {
        try {
      ...
      `);
            ...
            console.error('[Dashboard] top-projects error:', err.message ?? err);
            res.status(500).json({ error: 'Internal server error', detail: err.message });
        }
    });
    // GET /top-error-projects — top 10 projects by error count
    router.get('/top-error-projects', async (_req, res) => {
        try {
            const tables = await getProjectTables(pool);
            if (tables.length === 0)
                return res.json({ projects: [] });
            const parts = tables.map((t) => `SELECT REPLACE(project_name, '_', ' ') AS project_name, COUNT(*) AS cnt FROM "${t}" WHERE error IS NOT NULL AND error <> '' AND error_status IN ('open', 'reopened') GROUP BY project_name`);
      ...
      `);
            ...
            console.error('[Dashboard] top-error-projects error:', err.message ?? err);
            res.status(500).json({ error: 'Internal server error', detail: err.message });
        }
    });
    // GET /today-errors — all errors where timestamp is today (UTC)
    router.get('/today-errors', async (_req, res) => {
        try {
            const tables = await getProjectTables(pool);
            if (tables.length === 0)
                return res.json({ date: new Date().toISOString().slice(0, 10), errors: [] });
      ...
      `);
            ...
            console.error('[Dashboard] today-errors error:', err.message ?? err);
            res.status(500).json({ error: 'Internal server error', detail: err.message });
        }
    });
    ...
    router.get('/grouped', async (req, res) => {
      ...
      `;
            ...
            if (projectFilter)
                conditions.push(`project_name = '${projectFilter.replace(/'/g, "''")}'`);
 ...
 */
function createProjectErrorUpsertRouter(pool) {
    const express = require('express');
    const router = express.Router();
    // ── Teams webhook helper ──────────────────────────────────────────────────
    // Uses shared teamsNotifier — direct Teams Incoming Webhook
    function fireTeamsAlert(projectName, shortError, errorDetail) {
    ...
    router.post('/:name/errors', async (req, res) => {
        try {
            ...
            const { rows: tableCheck } = await pool.query('SELECT table_name FROM information_schema.tables WHERE table_schema = $1 AND LOWER(table_name) = LOWER($2)', ['public', tableNameRaw]);
            ...
            let isLive = false;
            try {
                const { rows: liveCheck } = await pool.query('SELECT is_live FROM projects WHERE name = $1', [projectName]);
            ...
            const errorDetail = body.error_detail?.trim() || undefined;
            ...
            if (!shortError)
                return res.status(400).json({ error: 'error or error_detail is required' });
            const hashSource = shortError.toLowerCase().trim();
            const errorHash = require('crypto').createHash('md5').update(hashSource).digest('hex');
            const updateResult = await pool.query('UPDATE "' + tableName + '" SET failure_count = failure_count + 1, file_name = $2, timestamp = NOW(), ' +
                ...
                'WHERE error_hash = $1 RETURNING id, error_status, failure_count', [errorHash, fileName, 'resolved', 'reopened', errorDetail ?? null]);
            if (updateResult.rowCount && updateResult.rowCount > 0) {
                const r = updateResult.rows[0];
                if (isLive)
                    fireTeamsAlert(projectName, shortError, errorDetail);
                return res.json({
            ...
            const insertResult = await pool.query('INSERT INTO "' + tableName + '" (id, project_name, file_name, timestamp, success_count, failure_count, ' +
                'error, error_detail, error_hash, error_status) ' +
                'VALUES ($7, $1, $2, NOW(), 0, 1, $3, $4, $5, $6) ' +
                'RETURNING id, error_status, failure_count', [projectName, fileName, shortError, errorDetail ?? null, errorHash, 'open', require('crypto').randomUUID()]);
    ...
    router.patch('/:name/errors/:hash/resolve', async (req, res) => {
        try {
            const projectName = decodeURIComponent(req.params.name);
            const tableNameRaw = projectName.replace(/ /g, '_');
            const errorHash = req.params.hash;
            ...
            const tableName = tableCheck[0].table_name;
            await pool.query('UPDATE "' + tableName + '" SET error_status = $1, resolved_at = NOW(), reopened_at = NULL WHERE error_hash = $2', ['resolved', errorHash]);
 ...
 */
function createProjectLiveRouter(pool) {
    const express = require('express');
    const router = express.Router();
    // ── Teams webhook (shared helper) ─────────────────────────────────────────
    // Uses shared teamsNotifier — direct Teams Incoming Webhook
    function fireTeamsAlert(projectName, shortError, errorDetail, label = '') {
        (0, teamsNotifier_1.sendTeamsAlert)({
    ...
    router.get('/live', async (_req, res) => {
        try {
            ...
            const { rows: tableExists } = await pool.query(`SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'projects' LIMIT 1`);
            if (tableExists.length === 0) {
                // No projects table — return all discovered project tables as "live"
                return res.json([]);
            }
            const { rows } = await pool.query('SELECT id, name, category, is_live FROM projects WHERE is_live = true ORDER BY name');
            res.json(rows);
        }
        catch (err) {
    ...
    router.patch('/:name/live', async (req, res) => {
        try {
            const projectName = decodeURIComponent(req.params.name);
            ...
            const { rows } = await pool.query('UPDATE projects SET is_live = $1 WHERE name = $2 RETURNING id, name, category, is_live', [is_live, projectName]);
            if (rows.length === 0) {
    ...
    router.post('/:name/test-error', async (req, res) => {
        try {
            ...
            const errorDetail = (body.error_detail ?? sample.error_detail).trim();
            const fileName = body.file_name ?? sample.file_name;
            // Derive short error from last line of detail
            const lines = errorDetail.split('\n').map((l) => l.trim()).filter(Boolean);
            const lastLine = lines[lines.length - 1] ?? '';
            const shortError = body.error ?? (lastLine.split(':')[0].trim() || sample.error);
            const crypto = require('crypto');
            const errorHash = crypto.createHash('md5').update(shortError.toLowerCase().trim()).digest('hex');
            // Upsert into project table
            const updateResult = await pool.query(`UPDATE "${tableName}"
         ...
         WHERE error_hash = $1
         RETURNING id, error_status, failure_count`, [errorHash, fileName, errorDetail]);
            let action;
            let errorStatus;
            let failureCount;
            if (updateResult.rowCount && updateResult.rowCount > 0) {
           ...
           RETURNING id, error_status, failure_count`, [projectName, fileName, shortError, errorDetail, errorHash, require('crypto').randomUUID()]);
            ...
            await (0, teamsNotifier_1.sendTeamsAlert)({
...
//# sourceMappingURL=projectDashboardRouter.js.map