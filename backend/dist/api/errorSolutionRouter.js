...
exports.createErrorSolutionRouter = createErrorSolutionRouter;
function createErrorSolutionRouter(pool) {
    ...
    router.post('/resolve', async (req, res) => {
        try {
            ...
            const { rows: tableCheck } = await pool.query(`SELECT table_name FROM information_schema.tables
         WHERE table_schema = 'public' AND LOWER(table_name) = LOWER($1)`, [tableNameRaw]);
            if (tableCheck.length === 0) {
                return res.status(404).json({ error: `No table found for project: ${project_name}` });
            }
            const tableName = tableCheck[0].table_name;
            const { rowCount } = await pool.query(`UPDATE "${tableName}"
         SET error_status = 'resolved', resolved_at = NOW()
         WHERE error_hash = $1 AND error_status IN ('open', 'reopened')`, [error_hash]);
    ...
    router.get('/:error_hash', async (req, res) => {
        try {
            const { error_hash } = req.params;
            const { rows } = await pool.query('SELECT solution, updated_at FROM error_solutions WHERE error_hash = $1', [error_hash]);
            if (rows.length === 0)
    ...
    router.post('/', async (req, res) => {
        try {
            ...
            if (!error_hash)
                return res.status(400).json({ error: 'error_hash is required' });
            const { rows } = await pool.query(`INSERT INTO error_solutions (error_hash, solution)
         VALUES ($1, $2)
         ON CONFLICT (error_hash)
         DO UPDATE SET solution = EXCLUDED.solution, updated_at = NOW()
         RETURNING *`, [error_hash, solution]);
            res.json(rows[0]);
        }
        catch (err) {
    ...
    router.delete('/:error_hash', async (req, res) => {
        try {
            const { error_hash } = req.params;
...
//# sourceMappingURL=errorSolutionRouter.js.map